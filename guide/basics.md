---
title: Основы работы с нашим API
description: Базовое руководство по авторизации и первым запросам
outline: deep
aside: false
---

<Proc :drag="true">
<pre>
set result 0
set result 10
set result 1000
</pre>
</Proc>

[//]: # ()
[//]: # (<Proc :drag="true">)

[//]: # (<pre>)

[//]: # (read result cell1 0)

[//]: # (write result cell1 0)

[//]: # (draw clear 0 0 0 0 0 0)

[//]: # (print "frog")

[//]: # (printchar 65)

[//]: # (format "frog")

[//]: # (drawflush display1)

[//]: # (printflush message1)

[//]: # (getlink result 0)

[//]: # (control enabled block1 0 0 0 0)

[//]: # (radar enemy any any distance turret1 1 result)

[//]: # (sensor result block1 @copper)

[//]: # (set result 0)

[//]: # (op add result a b)

[//]: # (select result notEqual x false a b)

[//]: # (lookup item result 0)

[//]: # (packcolor result 1 0 0 1)

[//]: # (unpackcolor r g b a color)

[//]: # (wait 0.5)

[//]: # (stop)

[//]: # (end)

[//]: # (jump -1 notEqual x false)

[//]: # (ubind @poly)

[//]: # (ucontrol move 0 0 0 0 0)

[//]: # (uradar enemy any any distance 0 1 result)

[//]: # (ulocate building core true @copper outx outy found building)

[//]: # (getblock block result 0 0)

[//]: # (setblock block @air 0 0 @derelict 0)

[//]: # (spawn @dagger 10 10 90 @sharded result)

[//]: # (status false wet unit 10)

[//]: # (weathersense result @rain)

[//]: # (weatherset @rain true)

[//]: # (spawnwave 10 10 false)

[//]: # (setrule waveSpacing 10 0 0 100 100)

[//]: # (message announce 3 @wait)

[//]: # (cutscene pan 100 100 0.06 0)

[//]: # (effect warn 0 0 2 %ffaaff )

[//]: # (explosion @crux 0 0 5 50 true true false true)

[//]: # (setrate 10)

[//]: # (fetch unit result @sharded 0 @conveyor)

[//]: # (sync var)

[//]: # (getflag result "flag")

[//]: # (setflag "flag" true)

[//]: # (setprop @copper block1 0)

[//]: # (playsound false @sfx-pew 1 1 0 @thisx @thisy true)

[//]: # (setmarker pos 0 0 0 0)

[//]: # (makemarker shape 0 0 0 true)

[//]: # (localeprint "name")

[//]: # (</pre>)

[//]: # (</Proc>)

Здесь начинается основной текст...

::: info Информация
Полезная информация или контекст. Заголовок "Информация" можно менять.
:::

::: tip Совет
Отличный способ сделать что-то быстрее.
:::

::: warning Внимание
Обратите на это внимание, чтобы избежать ошибок.
:::

::: danger Опасно
Действие может привести к потере данных.
:::

::: details Любой текст
Здесь можно спрятать длинный лог ошибки или дополнительный код.
:::


```javascript
const message = "Hello VitePress";
console.log(message);
```

```javascript {2,4}
function sayHi() {
console.log("Эта строка будет подсвечена");
let a = 1;
return a; 
}
```

